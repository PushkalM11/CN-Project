...
class VideoStreamRequestHandler:    
    ...
    async def stream(self):
        ...
        # logging.info(f"Stream Session {self.stream_id} has ended. Removing {self.cache_path}")

    async def send_frames(self):
        # logging.info("Beginning to send frames!")
        # logging.info(f"File to send: {self.file_to_serve}")
        ...
        while(cap.isOpened()):
            ...
            if ret == True:
                ...
                if flag:
                    ...
                    flag = False
                    self.transmit()
                # logging.info(f"StreamDataReceived: frame = {len(encoded_data)}")
    ...
    def quic_event_received(self, event: events.QuicEvent) -> None:
        if isinstance(event, StreamDataReceived):
            ...
            if query[0] == "POST":
                ...
                FOV = float(query[1])
                theta = float(query[2])
                # logging.info("theta just after receiving "+str(theta))
                alpha = float(query[3])
                phi = float(query[4])
